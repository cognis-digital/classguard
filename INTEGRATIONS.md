...
**classguard** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `classguard` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
classguard ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
classguard ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
classguard ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
classguard ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `classguard` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="classguard") for rec in records]   # records = your JSON output
