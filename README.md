...
[![PyPI](https://img.shields.io/pypi/v/cognis-classguard.svg)](https://pypi.org/project/cognis-classguard/)
[![CI](https://github.com/cognis-digital/classguard/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/classguard/actions)
...
CLASSGUARD exists for one job — validate classification banner markings (cui/confidential/secret) in documents per portion-marking rules. — and does it without a SaaS bill or heavyweight setup.
...
```bash
classguard --version
classguard scan .                      # scan the current project
classguard scan . --format json
classguard scan . --fail-on high       # non-zero exit for CI gates
classguard mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **ISOO marking guides**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`classguard` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
